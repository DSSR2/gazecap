...
parser = argparse.ArgumentParser(description='Split dataset based on orientation and device')
...
orientations = [' ', 'portrait', ' ',  'ls_home_right', 'ls_home_left']
...
def convert_dataset(files, out_root):
    for i in tqdm(files): 
        ...
        out_path = out_root+'/'+device+"/"+orientations[orientation]+"/"+ds+"/"
...
def preparePath(path, clear=True):
    ...
    for i in devices:
        os.mkdir(path+"/"+i+"/")
        os.mkdir(path+"/"+i+"/"+"portrait")
        add_ttv(path+"/"+i+"/"+"portrait")
        os.mkdir(path+"/"+i+"/"+"ls_home_left")
        add_ttv(path+"/"+i+"/"+"ls_home_left")
        os.mkdir(path+"/"+i+"/"+"ls_home_right")
        add_ttv(path+"/"+i+"/"+"ls_home_right")
    return path
